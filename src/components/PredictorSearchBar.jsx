...
function PredictorSearchBar() {
  ...
  const handleGemniSearchClick = async () => {
    ...
    try {
       const searchPrompt = getFixedPrompt({ dogBreed, dogAge, dogGender});
       const prompt=searchPrompt+userPrompt;

      const response=await axios.post(import.meta.env.VITE_BASE_URL+ "/gemniResponse"
        ,{
          prompt
        },
      ...
      setGeminiResponse(response?.data?.result);
